		...
		for (int dir = 0; dir < 4; ++dir)
		{
			...
			if( rcIsConnectedWalkableArea( cs, dir ) )
			{
...
#ifdef MODIFY_VOXEL_FLAG
				if( !rcIsSimilarTypeArea(chf.areas[ai], area) ) {
#else // MODIFY_VOXEL_FLAG
				...
				if( rcIsConnectedWalkableArea( as, dir2 ) )
				{
					...
					const int ai2 = (int)chf.cells[ax2+ay2*w].index + rcGetCon(as, dir2);
#ifdef MODIFY_VOXEL_FLAG
					if( !rcIsSimilarTypeArea(chf.areas[ai2], area) ) {
#else // MODIFY_VOXEL_FLAG
	...
	if (fillStack)
	{
		...
		for (int y = 0; y < h; ++y)
		{
			for (int x = 0; x < w; ++x)
			{
				...
				for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
				{
					if (chf.dist[i] >= level && srcReg[i] == 0 && rcCanMovableArea( chf.areas[i] ))
					{
						stack.push(x);
						stack.push(y);
						stack.push(i);
							  ...
							  unsigned short loglevelsPerStack) // the levels per stack (2 in our case) as a bit shift
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				if (!rcCanMovableArea( chf.areas[i] ) || srcReg[i] != 0)
					continue;
						...
						rcIntArray& cont)
{
	...
	if( rcIsConnectedWalkableArea( ss, dir ) )
	{
		const int ax = x + rcGetDirOffsetX(dir);
		const int ay = y + rcGetDirOffsetY(dir);
