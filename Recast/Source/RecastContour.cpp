						   ...
						   bool& isBorderVertex)
{
	...
	if( rcIsConnectedWalkableArea( s, dir ) )
	{
		...
		if( rcIsConnectedWalkableArea( as, dirp ) )
		{
	...
	if( rcIsConnectedWalkableArea( s, dirp ) )
	{
		...
		if( rcIsConnectedWalkableArea( as, dir ) )
		{
	...
	for (int j = 0; j < 4; ++j)
	{
		...
		const bool intsSameArea = rcIsSimilarTypeArea( (regs[c]>>16), (regs[d]>>16) );
