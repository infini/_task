...
protected:
	...
	struct RANGE_TILE_PER_VERTICES
	{
		int minCountTiles;
		int maxCountTiles;

		RANGE_TILE_PER_VERTICES() : minCountTiles( 0 ), maxCountTiles( 0 )	{}
		RANGE_TILE_PER_VERTICES( const int countTiles ) : minCountTiles( countTiles ), maxCountTiles( countTiles )	{}

		bool	operator == ( const RANGE_TILE_PER_VERTICES& rhs ) const
		{
			return rhs.minCountTiles <= minCountTiles && maxCountTiles <= rhs.maxCountTiles;
		}
	};
