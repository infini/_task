	...
	while(!done)
	{
		...
		if (showMenu)
		{
			...
			if (imguiCheck("Show Tools", showTools))
				showTools = !showTools;
#ifdef DIVISION_BUILD
			if( imguiCheck("Division Build", divisionBuild) ) {
				divisionBuild = !divisionBuild;
			}
#endif // DIVISION_BUILD
#ifdef INTEGRATION_BUILD
			if( imguiCheck("Integration Build", integrationBuild) ) {
...
#ifdef DIVISION_BUILD
			imguiSeparator();
			imguiLabel( "Input Directory" );
			if( imguiButton( directoryPath ) ) {
				if( showLevels ) {
					showLevels = false;
				}
				else {
...
#ifdef DIVISION_BUILD
					if( divisionBuild ) {
						scanDirectory( directoryPath, ".obj", files);
					}
					else {
			...
			if (geom && sample)
			{
				...
				if (imguiButton("Build"))
				{
					...
					if( divisionBuild ) {
						FileList meshFiles;
						scanDirectory( directoryPath, ".obj", meshFiles );
						...
						for( int nth = 0; nth < meshFiles.size; ++nth ) {
							...
							Sample* _sample = createTile();
							if( !_sample ) {
								continue;
							}
							InputGeom* _geom = new InputGeom;
							if( !_geom ) {
								delete _sample;
								continue;
							}
							...
							_sample->handleMeshChanged( _geom );
							_sample->handleSettings();
							
							result = _sample->handleBuild();
							if( result ) {
								_sample->save( path );
							}
							delete _sample;
						...
						result = sample->handleBuild();
					}
#else // DIVISION_BUILD
					const bool result = sample->handleBuild();
#endif // DIVISION_BUILD
					if( !result )
					{
						showLog = true;
			...
			if (levelToLoad != -1)
			{
				...
				if( divisionBuild ) {
					if( showDirectory ) {
						...
						::GetCurrentDirectory( _countof( directoryPath ), directoryPath );
						strcat( directoryPath, "\\Meshes\\" );
						strcat( directoryPath, files.files[levelToLoad] );
